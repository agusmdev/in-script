...
fetch(`https://n252jrjdnyzkxkxecnbmnhveme0sjlqi.lambda-url.us-east-1.on.aws/users/${userId}/paths`)
  ...
  .then(data => {
    ...
    const enabledPaths = data.enabled_paths;

    // Get the current URL of the page
    const currentUrl = window.location.href;

    // Check if the current URL is in the list of enabled domain paths
    if (!enabledPaths.includes(currentUrl) && !enabledPaths.includes("*")) {
      // Get the iframe element
      document.getElementById("interview-now-bubble-button").remove()
      document.getElementById("interview-now-bubble-window").remove()
    }
  })
